...
function setCrashed() { 
	...
	var fuellost = Math.round(((Math.random() * 20) + 20));
	...
	if(lander.fuel<1) { 
		...
		if(rnd < 0.3){
			...
			msg = "tu n'aura pas d'helium 3";
		}
		
		msg = "takasétamoto<br>" + fuellost + " FUEL UNITS LOST<br><br>" + msg;
			...
			if(line.landable) { 
				...
				if(lander.bottom>=line.p1.y) { 
					...
					if((left>line.p1.x) && (right<line.p2.x)) {
						...
						if((lander.rotation==0) && (lander.vel.y<0.25)) {
